...
# ---------------------------------------------------------------------------
# Per-label overview table (energy / time / scf steps) — improve.md Phase B
# ---------------------------------------------------------------------------
...
def render_overview_table(
    ...
    """Render the canonical per-label overview table.

    Every WorkChain's ``output_parameters`` carries, per child label (or
    pk), a ``total_energy``/``final_energy`` map (eV), a
    ``wall_time_seconds`` map and — since the parser unification — an
    ``scf_steps`` map. This single renderer produces the shared
    ``| label | energy (eV) | time (s) | scf steps |`` table so every
    report shows the same three quantities (improve.md §2.2).

    ``output_params`` is the workflow's output_parameters dict; the
    energy map is auto-detected as ``final_energy`` → ``total_energy``.
    Empty tables collapse to ``""`` (the caller skips the section).
    """
    energy_map = output_params.get("final_energy") or output_params.get(
        "total_energy"
    )
    if not energy_map:
        return ""

    time_map = output_params.get("wall_time_seconds") or {}
    steps_map = output_params.get("scf_steps") or {}
    status_map = output_params.get("status") or {}

    def _cell(mapping: Mapping[str, Any], key: str) -> str:
        value = mapping.get(key)
        if value is None or isinstance(value, str) and "failed" in str(value):
            return "—"
        return format_scalar(value, fmt="%.6f")

    lines = [
        "| label | energy (eV) | time (s) | scf steps | exit |",
        "| --- | --- | --- | --- | --- |",
    ]
    for label in energy_map:
        exit_val = status_map.get(label)
        exit_str = "—" if exit_val is None else str(int(exit_val))
        lines.append(
            f"| {label} | {_cell(energy_map, label)} | "
            f"{_cell(time_map, label)} | {_cell(steps_map, label)} | "
            f"{exit_str} |"
        )
    return "\n".join(lines)


def append_overview_section(lines: List[str], output_params: Dict[str, Any]) -> None:
    """Append the ``## Overview`` (energy/time/scf-steps per label) section.

    Every WorkChain report shows the same three quantities — energy (eV),
    wall time (s), scf steps — per child label. This appends the section
    in place when the output carries an energy map; otherwise it is a
    no-op (legacy outputs without the maps stay unchanged).
    """
    overview = render_overview_table(output_params)
    if overview:
